...
def get_links(n, g):
    ...
    if any(k in t for k in ["コフレ", "コスメ", "ホリデー"]):
        return [
            {"site_name": "meeco(三越伊勢丹)", "url": f"https://meeco.mistore.jp/meeco/search?q={e}", "deadline_date": None, "status": "未応募"},
            {"site_name": "阪急うめだコスメ", "url": f"https://web.hh-online.jp/hankyu-beauty/goods/list.html?shop=hb&keyword={e}", "deadline_date": None, "status": "未応募"},
            {"site_name": "アットコスメ", "url": f"https://www.cosme.com/products/list.php?name={e}", "deadline_date": None, "status": "未応募"}
        ]
    if any(k in t for k in ["TCG", "ポケカ", "ワンピ", "ドラゴンボール", "遊戯王"]):
        res = [{"site_name": "あみあみ（予約抽選）", "url": f"https://www.amiami.jp/top/page/c/search.html?s_keywords={e}", "deadline_date": None, "status": "未応募"},
               {"site_name": "ヨドバシ", "url": f"https://www.yodobashi.com/?word={e}", "deadline_date": None, "status": "未応募"},
               {"site_name": "スニダン相場", "url": f"https://snkrdunk.com/search?keywords={e}", "deadline_date": None, "status": "未応募"}]
        if "ポケ" in t: res.insert(0, {"site_name": "ポケセン", "url": f"https://www.pokemoncenter-online.com/?main_page=product_list&keyword={e}", "deadline_date": None, "status": "未応募"})
        return res
    if "プレバン" in t: return [{"site_name": "プレバン公式", "url": f"https://p-bandai.jp/chara/c0001/?keyword={e}", "deadline_date": None, "status": "未応募"}]
    if any(k in t for k in ["スニーカー", "NIKE", "SNKRS"]): return [{"site_name": "SNKRS", "url": f"https://www.nike.com/jp/w?q={e}", "deadline_date": None, "status": "未応募"}]
    if any(k in t for k in ["釣具", "DRT", "クラッシュ"]): return [{"site_name": "バックラッシュ", "url": f"https://www.backlash.co.jp/item_list/?kw={e}", "deadline_date": None, "status": "未応募"}]
    return [{"site_name": "公式情報元", "url": "https://google.com", "deadline_date": None, "status": "未応募"}]
...
def call_gemini(n, u, g, r):
    ...
    try:
        ...
        res = genai.Client(api_key=gk).models.generate_content(model="gemini-2.5-flash", contents=p)
...
def add_custom_and_create_card(name, raw_input):
    ...
    save_db({
        "id": str(int(time.time()*1000) + 1), "name": pn, "url": target_url, "retail_price": rp, "market_price": mp, "profit": pf,
        "margin_rate": mr, "break_even": int((rp+750)/0.9) if rp else 0, "sns_genre": "ホビー", "trust_score": 90, "trust_reason": "特設即時登録",
        "is_teaser": (rp==0 and mp==0), "created_at": td_str, "updated_at": td_str, "sites": sites
    })

def fetch_all_feeds(custom_list):
    hits = []
    qs = [("クリスマスコフレ 予約 抽選","コフレ"),("コスメデコルテ コフレ 予約 抽選","コフレ"),("Dior ホリデー 限定 予約","コフレ"),
          ("ポケカ 抽選予約 予約開始","TCG"),("ワンピースカード 抽選予約","TCG"),("プレミアムバンダイ 受注開始 限定","プレバン"),
          ("Nike SNKRS 抽選","スニーカー"),("ジャンプキャラクターズストア 受注","ホビー"),("TS-NEO ソフビ 抽選","ソフビ"),
          ("site:ameblo.jp タイニークラッシュ 抽選","釣具"),("DRT タイニークラッシュ 抽選","釣具"),
          ("ドラゴンボール カード 鑑定 PSA 落札","海外相場"),("ガレージブランド キャンプ 抽選","キャンプ")]
    for q, g in qs:
        ...
        for idx, s in enumerate(sites):
            with st.container(border=True):
                ...
                if nst != cur_st or (s.get("deadline_date") and nd != s.get("deadline_date")):
                    ...
                    st.rerun()
                st.link_button("👉 サイトを開く", clean_url(s.get("url", "https://google.com")), use_container_width=True)
